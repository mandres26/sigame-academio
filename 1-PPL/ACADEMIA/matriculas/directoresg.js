...
//$(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.d == null) {
                        ...
                        if (P.rol != "PROFESOR" || P.rol_terciario != "DIRECTOR_GRUPO") {
                            $("#formulario").hide();
                            $("#txtId").focus();
                            Modal("Al parecer esta identificación <strong> no le pertenece a un profesor director de grupo </strong> y no puede gestionar este proceso. Recuerde que debe de asignarle a un docente el rol de DIRECTOR_GRUPO si desea asignarlo como director.", "Esta identificación no le pertenece a un director de grupo");
                        } else if (P.rol == "PROFESOR" && P.rol_terciario == "DIRECTOR_GRUPO") {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        Modal("Error al intentar realizar esta acción. Al parecer este nombre <strong> no se encuentra registrado </strong> en la base de datos del sistema", "Error al intentar realizar esta acción");
                            ...
                            $("#formulario").hide();
                            $("#txtIdN").focus();
                            Modal("Error al intentar realizar esta acción. Al parecer este nombre <strong> no le pertenece a un profesor director de grupo </strong> y no puede gestionar este proceso. Recuerde que debe de asignarle a un docente el rol de DIRECTOR_GRUPO si desea asignarlo como director.", "Error al intentar realizar esta acción");
                            } else if (P.rol == "PROFESOR" && P.rol_terciario == "DIRECTOR_GRUPO") {
                            Alerta("¡Gestión realizada de <strong> forma exitosa</strong>!", "success");
                            $("#txtId").attr('disabled', 'disabled');
                            $("#txtIdN").attr('disabled', 'disabled');
    ...
    $('#btnR').click(function () { // Botón Registrar
            ...
            $.ajax(".....").always(function () {
                ...
                jsonData = "{'dto':" + JSON.stringify(getDatosM()) + ",'dtob':" + JSON.stringify(getBitacora("DIRECTOR_G", "REGISTRAR", "ACCIÓN REALIZADA A UN PROFESOR")) + "}"; //Registro de bitacora
            $.ajax({
                type: "POST",
                url: "/WS/matriculas.asmx/r_matriculaDirector",
                data: jsonData,
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                async: true,
                success: function (result) {
                    if (result.d == null) {
                        ...
                        Modal(JSON.stringify(result.d), "Asignación a director de grupo");
                        //nuevo();
                    }
                },
                error: function (result) {
    ...
    $('#btnE').click(function () { // Botón Eliminar
            ...
            $.ajax(".....").always(function () {
                ...
                jsonData = "{'dto':" + JSON.stringify(getDatosM()) + ",'dtob':" + JSON.stringify(getBitacora("DIRECTOR_G", "ELIMINAR", "ACCIÓN REALIZADA A UN PROFESOR")) + "}"; //Registro de bitacora
            $.ajax({
                type: "POST",
                url: "/WS/matriculas.asmx/e_matriculaDirector",
                data: jsonData,
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                async: true,
                success: function (result) {
                    if (result.d == null) {
                        Modal("Error al intentar realizar esta acción.", "No existe ninguna asignación para eliminarla");
                    }
                    else {
                        Modal(JSON.stringify(result.d), "Deshasignación de director de grupo");
                        nuevo();
                    }
                },
                error: function (result) {
