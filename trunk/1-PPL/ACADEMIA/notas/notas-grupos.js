...
//----------------------------------FUNCION PARA CALCUALR LA FECHA DE HOY------------------------------------------------------------------
    function fechadeHoy() {
        $.ajax({
            type: "POST",
            url: "/WS/periodos.asmx/c_fechaHoy",
...
//---------------------------------- --Cargamos a los profesores en el combo box-----------------------------------
 

//---------------------------------- --------------------------------------------------------------------------------------
    function Gdto() {
        var dto = {};
        dto.fhoyy = FHoy + "-" + "LOCALHOST";
       // dto.fhoyy = FHoy + "-" + "GODDADDY";
        dto.finicia = $("#txtRangoI").val();
        dto.ffin = $("#txtRangoF").val();
        return dto;
    }
    function FechasPeriodosPermisos() {
        jsonData = "{'dto':" + JSON.stringify(Gdto()) + "}";
        $.ajax({
            type: "POST",
            url: "/WS/periodos.asmx/c_fechasPerPer",
            data: jsonData,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
    ...
    function CargarProfes() {
        $.ajax({
            ...
            success: function (result) {
                ...
                $("#txtIdN").jqxComboBox({ selectedIndex: 0, source: PersonasAdapter, displayMember: "persona", promptText: "ESCRIBA O SELECCIONE...", valueMember: "persona", width: 370, height: 21 });
            },
            error: function (result) {
            }
        });
    ...
    $(document).on('click', '.clsAceptar2', function () {
        var btn = $(this)
        btn.button('loading')
        $.ajax(".....").always(function () {
