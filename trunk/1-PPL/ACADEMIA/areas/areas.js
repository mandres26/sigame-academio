...
//$(document).ready(function () {
    ...
    function Gestionar() {
        if ($('#txtId').val() == '') {
            Alerta("Error: Digite una área en el cuadro de texto y luego presione la tecla <strong> Enter</strong> o haga clic en el botón <strong> Gestionar </strong>para continuar.", "danger");
            $('#txtId').focus();
        } else {
    ...
    //});
    //function getCSVData() {
    //    var csv_value = $('#grilla').table2CSV({ delivery: 'value' });
    ...
    function nuevo() {
        ...
        $('#btnE').show();
        $('#btnL').show();
        Alerta("Digite la descripción del área en el cuadro de texto y luego presione la tecla <strong> Enter</strong> o haga clic en el botón <strong> Gestionar </strong>para continuar.", "info");
    }
    $('#btnR').click(function () { // Botón Registrar
        var btn = $(this)
        btn.button('loading')
        $.ajax(".....").always(function () {
            jsonData = "{'dto':" + JSON.stringify(getDatosG()) + ",'dtob':" + JSON.stringify(getBitacora("AREAS", "REGISTRAR", "NINGUNA")) + "}"; //Registro de bitacora
            $.ajax({
                type: "POST",
                url: "/WS/areas.asmx/r_area",
                data: jsonData,
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                async: true,
                success: function (result) {
                    if (result.d == null) {
                        ...
                        Modal(JSON.stringify(result.d), "Registro de áreas");
                        nuevo();
                    }
                },
                error: function (result) {
    ...
    $('#btnE').click(function () { // Botón Eliminar
        ...
        $.ajax(".....").always(function () {
            ...
            jsonData = "{'dto':" + JSON.stringify(getDatosG()) + ",'dtob':" + JSON.stringify(getBitacora("AREAS", "ELIMINAR", "NINGUNA")) + "}"; //Registro de bitacora
            $.ajax({
                type: "POST",
                url: "/WS/area.asmx/e_area",
                data: jsonData,
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                async: true,
                success: function (result) {
                    if (result.d == null) {
                        Modal("Error al intentar realizar esta acción.", "Verifique que la información que esta enviando sea la correcto o puede ser que el área no se encuentra registrado en el sistema. ");
                    }
                    else {
                        Modal(JSON.stringify(result.d), "Eliminación de áreas");
                        nuevo();
                    }
                },
                error: function (result) {
    ...
    $('#btnL').click(function () { // Botón Listar todos
        ...
        $.ajax(".....").always(function () {
            ...
            jsonData = "{'dtob':" + JSON.stringify(getBitacora("AREAS", "LISTAR TODOS", "NINGUNA")) + "}"; //Registro de bitacora
            $.ajax({
                type: "POST",
                url: "/WS/areas.asmx/c_areass",
                data: jsonData,
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                async: true,
    ...
    function titleGrilla() { // TITULO DE LA GRILLA
        ...
        newtitle += '<th class="col-sm-10 text-center">Áreas</th>';
        newtitle += "</tr>";
